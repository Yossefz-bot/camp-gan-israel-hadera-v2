...
const ALLOWED_KEYS = new Set([
  ...
  'show_testimonials','show_songs','show_countdown','allow_testimonial_submission','allow_newsletter_signup','allow_contact_form','floating_registration_enabled'
]);
...
export async function onRequest({ request, env }) {
  ...
  for (const [key, raw] of Object.entries(input)) {
    ...
    if (['show_testimonials','show_songs','show_countdown','allow_testimonial_submission','allow_newsletter_signup','allow_contact_form','hero_video_autoplay','hero_video_loop','hero_video_controls','story_video_autoplay','story_video_loop','story_video_controls','floating_registration_enabled'].includes(key)) value = value === '1' || value === 'true' ? '1' : '0';
